...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error('equipment_types query error:', error)
      return NextResponse.json({ error: 'Failed to fetch equipment types' }, { status: 500 })
    }

    return NextResponse.json({ items: data || [] })
  } catch (err: any) {
    console.error('GET /api/equipment-types error:', err)
    return NextResponse.json({ error: 'Server error' }, { status: 500 })
  }
}

// POST — create new equipment type (admin)
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error('equipment_types insert error:', error)
      if (error.code === '23505') {
    ...
    console.error('POST /api/equipment-types error:', err)
    return NextResponse.json({ error: 'Server error' }, { status: 500 })
  }
}
