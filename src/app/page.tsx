...
function DynamicBlock({ block, lang, index }: { block: PageBlock; lang: 'en' | 'ru'; index?: number }) {
  // ✅ STABILITY FIX: Memoize rendered HTML so that random IDs generated by renderers
  // stay the same between React re-renders (auth state, etc.). Without this, each re-render
  // creates new HTML → React replaces the DOM → observer's 'is-visible' class is lost →
  // sections with opacity:0 animations stay invisible.
  const safeContent = useMemo(() => {
    ...
    if (block.type === 'hero') {
      html = proxyImages(html, 800, 80) // Hero: 800px wide for 2x retina
    } else if (block.type === 'about') {
      html = proxyImages(html, 600, 78)
    } else {
      html = proxyImages(html, 1200, 80)
    }
