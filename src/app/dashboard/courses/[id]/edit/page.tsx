...
export default function CourseEditorPage() {
  ...
  const openAddLesson = (moduleId: string, type: 'video' | 'text' | 'task') => {
    ...
    setLessonForm({ title: '', title_ru: '', type, duration_minutes: '10', video_url: '', is_free: false })
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card><CardContent className="p-4 text-center">
          ...
          <div className="text-sm text-zinc-500">{ru ? 'Всего' : 'Total'}</div>
        </CardContent></Card>
      </div>
              ...
              <CardContent className="p-4 space-y-2">
                {mod.course_lessons?.length === 0 ? (
                  ...
                  mod.course_lessons?.map((lesson, li) => (
                    <div key={lesson.id} className="flex items-center gap-3 p-3 rounded-xl border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate">{ru && lesson.title_ru ? lesson.title_ru : lesson.title}</p>
                ...
                <div className="flex gap-2 pt-2">
                  <button onClick={() => openAddLesson(mod.id, 'video')} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-blue-600 hover:bg-blue-50 border border-blue-200">
                    <Video className="w-4 h-4" />{ru ? 'Видео' : 'Video'}
                  </button>
                  <button onClick={() => openAddLesson(mod.id, 'text')} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-green-600 hover:bg-green-50 border border-green-200">
                    <FileText className="w-4 h-4" />{ru ? 'Текст' : 'Text'}
                  </button>
                  <button onClick={() => openAddLesson(mod.id, 'task')} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-purple-600 hover:bg-purple-50 border border-purple-200">
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <Button variant="gradient" onClick={saveModule} disabled={saving}>
              {saving && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}{ru ? 'Сохранить' : 'Save'}
            </Button>
          </div>
        </div>
      </Modal>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {lessonForm.type === 'video' && (
              <Input label="Video URL" value={lessonForm.video_url} onChange={(e) => setLessonForm({ ...lessonForm, video_url: e.target.value })} placeholder="https://..." />
            )}
          </div>
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm">{ru ? 'Бесплатный урок' : 'Free lesson'}</span>
          </label>
          <div className="flex justify-end gap-3 pt-4">
