    ...
    try {
      const res = await fetchWithAuth('/api/form-templates', {
        ...
        body: JSON.stringify({
          ...
          type: template.type,
          fields: template.fields.map(f => ({ ...f, id: `f${++_fid}` })),
          active: false,
        }),
      })
      if (!res.ok) throw new Error()
              ...
              <Card><CardContent className="p-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    {templates.length > 1 && <Button variant="outline" size="sm" onClick={() => deleteTemplate(template.id)} title={ru ? 'Удалить' : 'Delete'}><Trash2 className="w-4 h-4 text-red-500" /></Button>}
                  </div>
                </div>
                <div className="flex items-center gap-3 mt-3">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <div className={`w-10 h-6 rounded-full transition-colors flex items-center px-0.5 ${template.active ? 'bg-teal-500' : 'bg-zinc-300'}`}
                    ...
                    <span className="text-sm text-zinc-600">{ru ? 'Активна' : 'Active'}</span>
                  </label>
                      ...
                      {isEditing && (
                        <div className="px-4 pb-4 pt-2 border-t border-zinc-100 space-y-4 bg-zinc-50/50">
                          ...
                          <label className="flex items-center gap-3 cursor-pointer">
                            ...
                            <span className="text-sm text-zinc-700">{ru ? 'Обязательное' : 'Required'}</span>
                          </label>
                        </div>
                      )}
                    </CardContent>
      ...
      {template && (
        <Modal isOpen={showPreview} onClose={() => setShowPreview(false)} title={ru ? 'Предпросмотр формы' : 'Form Preview'} size="lg">
          <div className="space-y-5 p-2">
            <h2 className="text-xl font-bold text-zinc-900">{ru ? template.name_ru : template.name_en}</h2>
            {fields.map(field => {
                ...
                <div key={field.id}>
                  ...
                  {field.type === 'scale' && (
                    <div className="flex gap-2 flex-wrap">{Array.from({ length: (field.max || 10) - (field.min || 1) + 1 }, (_, i) => (field.min || 1) + i).map(n => (
                      <button key={n} className="w-10 h-10 rounded-xl border border-zinc-200 text-sm font-medium text-zinc-500">{n}</button>
                    ))}</div>
                  )}
                  {field.type === 'select' && (
