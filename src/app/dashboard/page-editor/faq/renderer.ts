...
function sectionHeader(s: FaqSectionData, lang: 'en' | 'ru', align: string = 'center'): string {
  ...
  const titleStyle = v === 'gradient-text'
  ...
  return `<div style="text-align:${align};margin-bottom:clamp(32px,4vw,48px);">
    ${badgeH}
    ${title ? `<h2 style="font-size:clamp(26px,4vw,40px);font-weight:800;letter-spacing:-0.03em;line-height:1.15;${titleStyle}">${title}</h2>` : ''}
    ${sub ? `<p style="font-size:clamp(14px,2vw,17px);color:${txt};opacity:0.6;margin-top:12px;max-width:600px;${align === 'center' ? 'margin-left:auto;margin-right:auto;' : ''}">${sub}</p>` : ''}
  </div>`
}
  ...
  const items = s.items.map((item, i) => {
    ...
    return `<div class="fq-anim fq-acc" data-faq-idx="${i}" style="border-bottom:1px solid ${txt}11;">
      <button class="fq-acc-btn" onclick="(function(e){var p=e.closest('.fq-acc'),c=p.querySelector('.fq-acc-body'),ic=p.querySelector('.fq-acc-icon');if(c.style.maxHeight&&c.style.maxHeight!=='0px'){c.style.maxHeight='0px';c.style.opacity='0';ic.style.transform='rotate(0deg)'}else{c.style.maxHeight=c.scrollHeight+'px';c.style.opacity='1';ic.style.transform='rotate(45deg)'}})(this)" style="width:100%;display:flex;align-items:center;gap:14px;padding:20px 0;background:none;border:none;cursor:pointer;text-align:left;">
        ${prefix(s, i, item)}
        <span style="flex:1;font-size:clamp(15px,2vw,17px);font-weight:600;color:${txt};">${q}</span>
      ...
      <div class="fq-acc-body" style="max-height:${open ? '500px' : '0px'};opacity:${open ? '1' : '0'};overflow:hidden;transition:max-height 0.35s ease,opacity 0.3s ease;">
        <p style="padding:0 0 20px ${s.showNumbers || (s.showIcons && item.icon) ? '38px' : '0'};font-size:15px;line-height:1.7;color:${txt}aa;">${a}</p>
      </div>
    </div>`
  }).join('')
  ...
  const items = s.items.map((item, i) => {
    ...
    return `<div class="fq-anim fq-card" style="background:${cardBg};border:1px solid ${txt}0d;border-radius:20px;padding:clamp(24px,3vw,32px);transition:transform 0.3s,box-shadow 0.3s;">
      <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:12px;">
        ${prefix(s, i, item)}
        <h3 style="font-size:16px;font-weight:700;color:${txt};line-height:1.4;">${q}</h3>
      </div>
      <p style="font-size:14px;line-height:1.7;color:${txt}99;${s.showNumbers || (s.showIcons && item.icon) ? 'padding-left:36px;' : ''}">${a}</p>
    </div>`
  }).join('')
  ...
  const items = s.items.map((item, i) => {
    ...
    return `<div class="fq-anim" style="padding:24px 0;border-bottom:1px solid ${txt}0d;">
      <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:8px;">
        ${prefix(s, i, item)}
        <h3 style="font-size:15px;font-weight:700;color:${txt};">${q}</h3>
      </div>
      <p style="font-size:14px;line-height:1.7;color:${txt}88;${s.showNumbers || (s.showIcons && item.icon) ? 'padding-left:34px;' : ''}">${a}</p>
    </div>`
  }).join('')
  ...
  const items = s.items.map((item, i) => {
    ...
    return `<div class="fq-anim fq-acc" data-faq-idx="${i}" style="border-bottom:1px solid ${txt}11;">
      <button class="fq-acc-btn" onclick="(function(e){var p=e.closest('.fq-acc'),c=p.querySelector('.fq-acc-body'),ic=p.querySelector('.fq-acc-icon');if(c.style.maxHeight&&c.style.maxHeight!=='0px'){c.style.maxHeight='0px';c.style.opacity='0';ic.style.transform='rotate(0deg)'}else{c.style.maxHeight=c.scrollHeight+'px';c.style.opacity='1';ic.style.transform='rotate(45deg)'}})(this)" style="width:100%;display:flex;align-items:center;gap:14px;padding:18px 0;background:none;border:none;cursor:pointer;text-align:left;">
        ${prefix(s, i, item)}
        <span style="flex:1;font-size:15px;font-weight:600;color:${txt};">${q}</span>
      ...
      <div class="fq-acc-body" style="max-height:${open ? '500px' : '0px'};opacity:${open ? '1' : '0'};overflow:hidden;transition:max-height 0.35s ease,opacity 0.3s ease;">
        <p style="padding:0 0 18px ${s.showNumbers || (s.showIcons && item.icon) ? '38px' : '0'};font-size:14px;line-height:1.7;color:${txt}aa;">${a}</p>
      </div>
    </div>`
  }).join('')
